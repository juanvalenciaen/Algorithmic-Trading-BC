...
input int Stop_Loss   = 73; // Stop Loss (pips)
input int Take_Profit = 0; // Take Profit (pips)
static input string Ind0 = "------------";// ----- Bollinger Bands -----
input int Ind0Param0 = 36; // Period
input double Ind0Param1 = 3.72; // Deviation
static input string Ind1 = "------------";// ----- Stochastic Signal -----
input int Ind1Param0 = 26; // %K Period
input int Ind1Param1 = 3; // %D Period
input int Ind1Param2 = 2; // Slowing
static input string Ind2 = "------------";// ----- Moving Averages Crossover -----
input int Ind2Param0 = 5; // Fast MA period
input int Ind2Param1 = 10; // Slow MA period
static input string Ind3 = "------------";// ----- Stochastic Signal -----
input int Ind3Param0 = 29; // %K Period
input int Ind3Param1 = 16; // %D Period
input int Ind3Param2 = 7; // Slowing

static input string ExpertSettings__ = "------------"; // ------ Expert Settings ------
static input int Magic_Number = 18351616; // Magic Number
...
int OnInit()
  {
   ...
   ind0handler = iBands(NULL,0,Ind0Param0,0,Ind0Param1,PRICE_CLOSE);
   ind1handler = iStochastic(NULL,0,Ind1Param0,Ind1Param1,Ind1Param2,MODE_SMA,STO_LOWHIGH);
   ind2handler = iMA(NULL,0,Ind2Param0,0,MODE_SMA,PRICE_CLOSE);
   ind21handler= iMA(NULL,0,Ind2Param1,0,MODE_SMA,PRICE_CLOSE);
   ind3handler = iStochastic(NULL,0,Ind3Param0,Ind3Param1,Ind3Param2,MODE_SMA,STO_LOWHIGH);
...
void ManageOpen()
  {
   double ind0buffer0[]; CopyBuffer(ind0handler,1,1,2,ind0buffer0);
   double ind0buffer1[]; CopyBuffer(ind0handler,2,1,2,ind0buffer1);
   ...
   double ind1buffer0[]; CopyBuffer(ind1handler,MAIN_LINE,1,2,ind1buffer0);
   double ind1buffer1[]; CopyBuffer(ind1handler,SIGNAL_LINE,1,2,ind1buffer1);
...
void ManageClose()
  {
   ...
   double ind2buffer1[]; CopyBuffer(ind21handler,0,1,2,ind2buffer1);
   ...
   double ind3buffer0[]; CopyBuffer(ind3handler,MAIN_LINE,1,2,ind3buffer0);
   double ind3buffer1[]; CopyBuffer(ind3handler,SIGNAL_LINE,1,2,ind3buffer1);
...
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"stopLoss":73,"takeProfit":0,"useStopLoss":true,"useTakeProfit":false,"isTrailingStop":false},"openFilters":[{"name":"Bollinger Bands","listIndexes":[5,3,0,0,0],"numValues":[36,3.72,0,0,0,0]},{"name":"Stochastic Signal","listIndexes":[0,0,0,0,0],"numValues":[26,3,2,0,0,0]}],"closeFilters":[{"name":"Moving Averages Crossover","listIndexes":[1,0,0,0,0],"numValues":[5,10,0,0,0,0]},{"name":"Stochastic Signal","listIndexes":[2,0,0,0,0],"numValues":[29,16,7,0,0,0]}]} */
